...
class CustomCtaAllOptions extends Component {
  ...
  _renderButton() {
    ...
    const buttonIcon = props.button_icon
  ...
  _renderProp(value, fieldName, fieldType, renderSlug) {
    ...
    switch (fieldType) {
      case 'options_list':
        ...
        if (_.isArray(value)) {
          output = value.map((option, index) => {
            return (
              <option key={`${orderClass}-${index}`} value={option.value}>
                {option.value}
              </option>
            );
          });
      ...
      case 'select_fonticon':
        output = (
          <span style={{ fontFamily: '"ETmodules"', fontSize: 40 }}>
